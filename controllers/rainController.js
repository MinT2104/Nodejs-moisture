...
const rainController = {
  ...
  addFeed: async (req, res) => {
    try {
      if (req.body.data !== undefined) {
        ...
        const rainChosen = await rain.findOneAndUpdate(
          { pid: req.body.pid },
          { $push: { feeds: newObject } },
          { new: true }
        );
  ...
  delAnItemFeed: async (req, res) => {
    try {
      const newRain = await rain.findOneAndUpdate(
        { espName: req.body.espName },
        { $pull: { feeds: { entryId: req.body.entryId } } },
        { new: true }
      );
